...
data_end = "06.05.2021"
logger.info('Выбрана дата окончания трехмесячного диапазона')
...
def filter_period(df: pd.DataFrame, data_end: str = current_date) -> pd.DataFrame:
    ...
    logger.info('Датафрейм по диапазону дат отфильтрован')
    return df_filtr
...
def spending_by_category(df_filter: pd.DataFrame, category: str) -> pd.DataFrame:
    ...
    logger.info('Расходы по выбранной категории подсчитаны')
